---
title: "Predicting International Happiness Scores"
author: "Alex Bender"
date: "April 17, 2019"
output:
  word_document: default
  html_document: default
email: bender.a@husky.neu.edu
...
The world happiness dataset has `r nrow(wh2016)` countries. 
...
nrow(world_df)
```
`r nrow(world_df)` countries remaining, perfect!
...
The RMSE of this model is `r RMSE`. Which isn't the most meaningful on it's own but would be more meaningful when compared to other models. However, this is saying that on average, the square error between actual and predicted squared is `r RMSE` for happiness score.

Let's also calculate the Mean Absolute Error. This is also a measure of fit and accuracy of a regression model.

```{r echo=FALSE}
#calculate the MAE
aaa <- mean(abs(m.mlreg$residuals))
```
This is saying that on average the predicted value from the regression model is `r aaa` off the actual value. This seems very good! 
...
```
The MAE is `r MAE(pred.regtree, world_test2$Happiness.Score)`. This isn't super meaningful until we have a reference to compare it to (another model). However, this does tell us that on average our model is `r MAE(pred.regtree, world_test2$Happiness.Score)` happiness score away from the actual value.
...
```
The MAE in this instance is `r MAE(avg, world_test2$Happiness.Score)`, so my model is definitely better than only guessing the mean. 
...
`r cor(pred.nn, world_test3$Happiness.Score)` is a pretty strong correlation!  
...
```
Adding 5 hidden nodes actually made the prediction performance go down to `r cor(pred.nn2, world_test3$Happiness.Score)`. The added complexity did not help the model. Different random seeds will have varying results. 

```{r cor(pred.nn2, world_test3$Happiness.Score)}
...
I am choosing to start with k = 11 because that is close to the square root of `r nrow(world_train4)`, which is the size of the cases in the training data set. In practice, it would be necessary to try several different values of k in order to arrive at the best one. In this instance, I made the decision to just predict using the k=11. 
...
rmse_mlreg
```
The average MAE between the 10 folds for the Multiple Linear Regression Model is `r mae_mlreg` and the average RMSE is `r rmse_mlreg`. 
...
rmse_regtree
```
The average MAE between the 10 folds for the Regression Tree Model is `r mae_regtree` and the average RMSE is `r rmse_regtree`. 
...
rmse_regtree2
```
The average MAE between the 10 folds for the Regression Tree Model is `r mae_regtree2` and the average RMSE is `r rmse_regtree2`. 
...
rmse_nn
```
This neural network model gives an average MAE of `r mae_nn` and an average RMSE of `r rmse_nn`.
...
rmse_nn2
```
This neural network model gives an average MAE of `r mae_nn2` and an average RMSE of `r rmse_nn2`.
...
ggplot(data = viz, aes(x = Metric, y = Model)) + geom_tile(aes(fill = Value)) + scale_fill_gradient(low = "#77dd77", high = "#fdfd96") + geom_text(aes(label = round(Value,2)))
```
As you can see, the Ensemble now has the best performance with a MAE of `r MAE_e1` and a RMSE of `r RMSE_e1`. 
...
RMSE_e2
```
The simple averaging ensemble had a lower MAE and RMSE. The stacked model had a MAE of `r MAE_e2` and a RMSE of `r RMSE_e2`. Now let's add this to the heatmap. 
